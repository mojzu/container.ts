...
gulp.task("distclean", ["clean"], (done) => {
  clean.run(config.package.path, ["node_modules"], done);
});
...
gulp.task("test", ["tsc"], (done) => {
  shell.run("istanbul cover dist/test.js -x \"**/*.spec.js\"", config.package.path, done);
});
...
gulp.task("build", ["assets", "test", "lint", "webpack"], (done) => {
