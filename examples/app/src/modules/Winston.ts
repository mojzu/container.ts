  ...
  protected logsOnMessage(log: ContainerLogMessage): void {
    ...
    switch (log.level) {
      case ELogLevel.Emergency: {
        this.logger.emerg(message, log.metadata, ...args, callback);
        break;
      }
      case ELogLevel.Alert: {
        this.logger.alert(message, log.metadata, ...args, callback);
        break;
      }
      case ELogLevel.Critical: {
        this.logger.crit(message, log.metadata, ...args, callback);
        break;
      }
      case ELogLevel.Error: {
        this.logger.error(message, log.metadata, ...args, callback);
        break;
      }
      case ELogLevel.Warning: {
        this.logger.warning(message, log.metadata, ...args, callback);
        break;
      }
      case ELogLevel.Notice: {
        this.logger.notice(message, log.metadata, ...args, callback);
        break;
      }
      case ELogLevel.Informational: {
        this.logger.info(message, log.metadata, ...args, callback);
        break;
      }
      case ELogLevel.Debug: {
        this.logger.debug(message, log.metadata, ...args, callback);
        break;
      }
    }
  }
