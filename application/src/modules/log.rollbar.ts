  ...
  protected handleLog(log: ContainerLogMessage): void {
    ...
    switch (log.level) {
      ...
      case LogLevel.Critical: {
        this._rollbar.critical(log.message, ...log.args, callback);
        break;
      }
      case LogLevel.Error: {
        this._rollbar.error(log.message, ...log.args, callback);
        break;
      }
      case LogLevel.Warning: {
        this._rollbar.warning(log.message, ...log.args, callback);
        break;
      }
      case LogLevel.Notice:
      case LogLevel.Informational: {
        this._rollbar.info(log.message, ...log.args, callback);
        break;
      }
      case LogLevel.Debug: {
        this._rollbar.debug(log.message, ...log.args, callback);
        break;
      }
    }
  }
